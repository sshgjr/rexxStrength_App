...
나쁜 예시 (절대 이렇게 쓰지 말 것):
  summary: "사이드프레셔 동작을 위한 준비 자세를 점검해 볼까요?" ← 너무 형식적
...
팔씨름 운동 피드백에는 반드시 실전 연결을 포함해라:
  - "실전에서 상대에게 손등을 내주지 않고 압박할 수 있어요"
  - "실전 팔씨름 사이드프레셔 패턴과 일치해요"
  - "고중량을 다룰 때 이 부분이 먼저 무너질 수 있어요"
  - "이 자세가 잡히면 실전에서 훨씬 강한 압박이 나와요"
...
원인을 해결하는 구체적 행동:
  - 신체 부위 + 방향 + 감각적 표현 포함
  - 예: "엄지를 바닥 방향으로 완전히 돌려서 회내 상태를 먼저 만들고, 팔꿈치를 옆구리에 바짝 붙이며 밀어보세요"
  - 예: "팔꿈치와 몸이 하나의 덩어리가 되는 느낌으로 회전해보세요"

[cue 작성 규칙]
"GOOD 파트 / 개선 파트" 형식:
  예: "프레임 고정 GOOD / 손목 회내 + 팔꿈치 밀착 필요"
  예: "체중 전달 GOOD / 손목 커핑 강화 필요"
    ...
    fallback_map = {
        "side_pressure": {
            ...
            "low": {
                ...
                "reason": f"사이드프레셔는 팔꿈치 90도 + 손목 회내가 핵심이에요.\n{weak_detail or '팔꿈치가 펴지거나 손목이 회내되지 않으면 힘이 어깨와 위쪽으로 분산되어 사이드 방향으로 전달이 안 돼요. 실전에서 상대를 제대로 밀어낼 수 없어요.'}",
                "action": "가벼운 무게로 팔꿈치를 90도로 굽히고, 엄지를 완전히 아래로 돌린 회내 상태에서 옆으로 미는 패턴을 먼저 익혀보세요. 이 감각이 잡히면 중량을 올려도 힘이 그대로 유지돼요.",
        ...
        "wrist_curl": {
            ...
            "mid": {
                ...
                "reason": f"{strong_detail or '핑거롤 방식으로 손가락 끝까지 힘을 전달하려는 시도가 좋아요. 전완 전체를 동원하는 방향으로 가고 있어요.'}\n{weak_detail or '손목 가동범위가 짧아서 전완 굴곡근의 수축 범위가 줄어들고 있어요. 이 상태에서는 최대 근력의 절반도 못 쓰는 경우가 많아요.'}",
                "action": "손가락을 끝까지 펴서 바를 손끝으로 받치는 느낌으로 내리고, 손목을 끝까지 꺾어 올려보세요. 이 Full ROM이 전완 굴곡근을 완전히 자극해요.",
